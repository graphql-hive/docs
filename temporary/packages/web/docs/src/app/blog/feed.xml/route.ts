...
function getAuthor(
  frontmatterAuthors: BlogFrontmatter["authors"],
): string | undefined {
  const first = Array.isArray(frontmatterAuthors)
    ? frontmatterAuthors[0]
    : frontmatterAuthors;
...
export async function GET() {
  ...
  for (const page of pagesDepthFirst(pages)) {
    ...
    switch (dir) {
      ...
      case "case-studies":
        allPosts.push(
          toRssItem(coerceCaseStudyToBlog(parseSchema(page, CaseStudyFile))),
        );
        break;
    }
  }
