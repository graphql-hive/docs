...
export type HiveNavigationProps = {
  ...
  /**
   * Used to determine bidirectional links (e.g., landing page <-> docs).
   */
  pathname: string;
  productName: string;
  search?: ReactElement;
...
export function HiveNavigation({
  ...
  const defaultLogo = <HiveLogoLink isHive={productName === 'Hive'} />;

  return (
    <div
      className="sticky top-0 z-20 border-b border-beige-400/[var(--border-opacity)] bg-[rgb(var(--nextra-bg,255,255,255))] px-6 py-4 text-green-1000 transition-[border-color] duration-500 md:mb-[7px] md:mt-2 dark:border-neutral-700/[var(--border-opacity)] dark:text-neutral-200"
      ref={containerRef}
      ...
      >
        ...
        <NavigationMenuList className="ml-4 bg-white dark:bg-transparent [@media(min-width:1180px)]:ml-16">
          <NavigationMenuItem>
            ...
            <NavigationMenuContent>
              <ProductsMenu pathname={pathname} productName={productName} />
            </NavigationMenuContent>
          </NavigationMenuItem>
          <NavigationMenuItem>
      ...
      <MenuContentColumns ref={ref} {...rest}>
        ...
        <div className="w-[364px]">
          ...
          <ul className="grid grid-cols-2 gap-x-4">
            {SIX_HIGHLIGHTED_PRODUCTS.map((product: (typeof SIX_HIGHLIGHTED_PRODUCTS)[number]) => {
              const Logo = product.logo;
              return (
                <li key={product.name}>
                  <NavigationMenuLink
...
function HamburgerButton({
  ...
  return (
    <button
      ...
      type="button"
    >
      <MenuIcon className={cn({ open: menuOpen }, 'size-6 stroke-current')} />
    </button>
  );
}
