...
async function collectDevStyles(options: {
  ...
  for (const entry of options.entries) {
    const normalizedPath = entry.replaceAll("\\", "/");
    ...
    if (!node) {
      try {
        ...
        // Ignore missing transient modules in dev graph.
      }
...
async function crawlModuleForCss(
  ...
  if (!node.ssrTransformResult) {
    try {
      ...
      // Ignore modules that cannot be transformed during graph crawl.
    }
  }
      ...
      viteDevServer.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          viteDevServer.config.logger.error(
            `[hive] Error collecting dev styles: ${
