...
<Callout type="warning">
  In case you want to use Apollo-Server as a Federation gateway, please refer to
  the [Apollo Gateway integration guide](./apollo-gateway.mdx).
</Callout>
...
<Callout type="info">
  ...
  project, because it includes a runtime to send usage reports to the Hive
  registry.
</Callout>
...
["Configuration"](/docs/api-reference/client#configuration) page.

## Integration

### Publishing Schemas

Please use the [Hive CLI to publish your GraphQL schema](/docs/api-reference/cli#publish-a-schema).
...
<Callout>
  For more configuration options, such as sampling, please refer to the [Hive
  client configuration](/docs/api-reference/client#configuration) reference.
</Callout>
...
```ts title="Node.js Apollo Server S setup" {8-12,19-21}
...
you need to send the client information HTTP headers for requests to the Apollo Server instance from
your client. Those are `x-graphql-client-name` (or `graphql-client-name`) and
`x-graphql-client-version` (or `graphql-client-version`). You must include both of these headers,
otherwise the data will not be passed along.

```bash title="Example HTTP Request with client headers" {2-3}
curl \
...
```ts title="Node.js Apollo Server WebSocket setup" {4,9,22-26,31,32,59}
...
```ts title="GraphQL over WebSocket client configuration"
