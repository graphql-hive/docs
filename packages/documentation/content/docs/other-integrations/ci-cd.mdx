...
Pass `--base` with a `<revision>:<path>` value to compare the checked-out
      ...
      - name: schema check
        ...
        run: |
          ...
          hive schema:check "path/to/schema.graphql" \
            ...
            --base "$MERGE_GROUP_BASE_SHA:path/to/schema.graphql" \
            --github
```
...
The `--base` option also accepts a local schema file. When using the Git revision syntax shown above,
