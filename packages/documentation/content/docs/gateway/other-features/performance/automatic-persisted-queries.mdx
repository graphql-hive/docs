...
```ts title="Automatic Persisted Queries Gateway setup" {3, 13}
...
```ts title="Automatic Persisted Operations with a custom store"
import { defineConfig } from "@graphql-hive/gateway";
...
subgraph does not recognise the hash) and mutations always continue to use POST.

```ts title="APQ with GET for hashed queries"
import { defineConfig } from "@graphql-hive/gateway";
...
Some upstream servers require a `content-type: application/json` header even on GET requests. You
can enable this with `useContentTypeForGETRequests`:

```ts title="APQ with GET and content-type header"
import { defineConfig } from "@graphql-hive/gateway";
...
- [APQ with Apollo Server subgraph](https://www.apollographql.com/docs/apollo-server/performance/apq)
