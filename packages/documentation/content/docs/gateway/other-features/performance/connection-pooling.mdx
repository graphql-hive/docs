...
Use `timeout` to set a socket-level inactivity timeout (in milliseconds). This causes the socket to
emit a `timeout` event if no data is received for the specified duration, which can help surface
unresponsive subgraphs quickly instead of waiting indefinitely.
...
const agent = new http.Agent({
  keepAlive: true,
  maxSockets: 50,
  timeout: 5000, // emit 'timeout' if socket is idle for 5 s
});
...
<Callout>
  ...
  see the [Secure HTTP Connection (HTTPS)](/docs/gateway/other-features/security/https) page.
</Callout>
