...
Use `identifier: "{args.*}"` to create a separate rate limit bucket per unique argument value. This is useful for unauthenticated endpoints where you want to limit access per resource ID rather than per caller:
...
Use `identityArgs` to append one or more argument values to the identity key. This is equivalent to `identifier: "{args.*}"` but supports multiple arguments and dot-notation paths:
...
You can also combine argument and context values by using a custom `identifyFn`, which receives the resolved argument values as a second parameter:
...
| `window`       | `string`                | Duration of the time window as a human-readable string (e.g. `"1s"`, `"30s"`, `"1m"`, `"1h"`). Alternative to `ttl`.                                                                                      |
| `identifier`   | `string`                | Template string for identifying the caller. Supports `{context.*}` for request context values and `{args.*}` for field argument values.                                                                   |
| `identifyFn`   | `(ctx, args) => string` | Function that returns the caller identity key. Receives the context and the resolved field argument values. Takes precedence over `identifier` when both are set.                                         |
| `identityArgs` | `string[]`              | Field argument names whose values are appended to the identity key, creating a separate bucket per unique combination of argument values. Supports dot notation for nested paths (e.g. `"input.userId"`). |
