...
1. `authorization` header (via `context.headers` or `context.request.headers`)
2. Remote IP address (via `context.req.socket.remoteAddress`, `context.req.connection.remoteAddress`, or `context.req.ip`)
3. `x-forwarded-for` header (via `context.request.headers` or `context.headers`)
