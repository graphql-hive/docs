...
When you only need to limit a field by caller without caring about arguments, use `identifier` with a `{context.*}` path. The gateway automatically tries common request properties (`authorization` header, remote address, `x-forwarded-for`, etc.), so for most cases you only need to specify the field limits:
