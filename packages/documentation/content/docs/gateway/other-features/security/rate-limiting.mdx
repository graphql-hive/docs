...
When using Apollo Federation, you can annotate fields directly in your subgraph schemas with the
...
type Mutation {
  createOrder(input: OrderInput!): Order
    @rateLimit(window: "1m", max: 5, message: "Too many orders, please slow down.")
}
```
...
<Callout>
  ...
  [Performance & Caching](/docs/gateway/other-features/performance) page for
  all available cache backends and configuration options.
</Callout>
