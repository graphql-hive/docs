...
## Controlling Preflight Response Headers

The `headers` configuration block does not affect CORS preflight (`OPTIONS`) responses — those are
returned early by the CORS layer before any header-injection middleware runs. Use
`preflight_response_headers` to attach arbitrary headers (e.g. `Cache-Control`, `Server-Timing`,
custom `X-*` headers) to those responses.
...
You can also set per-policy preflight headers that are merged on top of the global map:
...
cors:
  ...
  max_age: 3600
  preflight_response_headers:
    Cache-Control: "public, max-age=3600" # global default
  policies:
    - origins:
        - https://my-app.com
      preflight_response_headers:
        Cache-Control: "private, max-age=300" # overrides global for this origin
