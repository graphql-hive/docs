...
## Customize Preflight (`OPTIONS`) Responses

If you need to control headers returned in preflight responses, use
`preflight_response_headers`.

This is useful when you want browser/CDN caching hints (for example, `Cache-Control`) or
additional metadata headers on `OPTIONS` responses.
