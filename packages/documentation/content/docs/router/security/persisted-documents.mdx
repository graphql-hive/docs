...
By default, if `selectors` is not set, Router first checks `documentId` (request body for POST,
query parameter for GET), then `extensions.persistedQuery.sha256Hash` (Apollo format). You can
customize this order and use
...
If you are using Apollo Client and its [`clientAwareness` feature](https://www.apollographql.com/docs/react/api/link/apollo-link-client-awareness#configuring-with-apollo-client) or [Apollo Kotlin](https://www.apollographql.com/docs/kotlin/advanced/client-awareness), configure the name and version headers to match the defaults sent by Apollo clients and send only a plain document ID:
