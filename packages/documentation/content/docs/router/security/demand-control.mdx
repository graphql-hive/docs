...
2. **Limit checking**: If estimated cost exceeds configured limits (`strategy.static_estimated.max`
   globally or subgraph-specific `max`), the router can reject the operation or skip over-budget
   subgraphs while continuing others.
...
Operations exceeding `strategy.static_estimated.max` respond with:
  ...
  strategy:
    static_estimated:
      ...
      subgraph:
        ...
        all:
          max: 1000 # Any subgraph can use up to 1000 cost
          list_size: 20

        # Override for specific subgraphs
        subgraphs:
          search_engine:
            max: 200 # Search is expensive, stricter limit
            list_size: 5

          analytics:
            max: 500 # Analytics can handle more
...
```jsonc
{
  ...
  "extensions": {
    "cost": {
      ...
      "resultBySubgraph": {
        "users": "COST_OK",
        "search_engine": "SUBGRAPH_COST_ESTIMATED_TOO_EXPENSIVE",
        "products": "COST_OK",
      },
    },
  },
...
- `actual`: Post-execution cost (always computed; method controlled by `actual_cost_mode`)
...
- `estimatedCostBySubgraph`/`actualCostBySubgraph`: Per-subgraph cost breakdown
- `resultBySubgraph`: Per-subgraph result codes (for example `COST_OK` or
  `SUBGRAPH_COST_ESTIMATED_TOO_EXPENSIVE`)
- `estimatedFormulaBySubgraph`: Diagnostic per-subgraph cost formulas used during estimation
...
demand_control:
  ...
  strategy:
    static_estimated:
      max: 500
      actual_cost_mode: by_subgraph # or by_response_shape
  include_extension_metadata: true
```
...
When running in `mode: enforce`, operations that exceed `strategy.static_estimated.max` during
actual execution append an error to the response:
...
```jsonc
{
  ...
  "errors": [
    {
      "message": "Operation actual cost 527 exceeds configured max cost 500",
      "extensions": {
...
| `COST_ESTIMATED_TOO_EXPENSIVE`          | Pre-execution  | Estimated cost exceeds global `max`   | Request rejected, no subgraph calls made                |
...
| `COST_ACTUAL_TOO_EXPENSIVE`             | Post-execution | Actual cost exceeds global `max`      | Data is returned with an additional GraphQL error       |
...
- Enable Demand Control in `mode: measure`
...
- Set `strategy.static_estimated.max` to the 99th percentile of observed costs
...
- Gradually lower `strategy.static_estimated.max` as developers optimize queries
