...
stateDiagram-v2
  [*] --> Closed
  Closed --> Open: rolling error rate over the last<br/>volume_threshold calls ≥ error_threshold
  Open --> HalfOpen: reset_timeout elapsed
  HalfOpen --> Closed: probe error rate over the last<br/>half_open_attempts probes < error_threshold
  HalfOpen --> Open: probe error rate over the last<br/>half_open_attempts probes ≥ error_threshold
```
...
<Callout type="info">
  When you provide `error_status_codes` in a per-subgraph override, the override **fully replaces**
