...
Server-Sent Events (SSE) is a simple, efficient protocol built on standard HTTP. The router supports SSE for both client-to-router and router-to-subgraph communication.
...
HTTP subscriptions are enabled automatically when [subscriptions are enabled](/docs/router/subscriptions#configuration). No additional configuration is needed to use SSE on the client side.
