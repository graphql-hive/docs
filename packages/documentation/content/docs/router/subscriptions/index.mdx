...
When dealing with HTTP subscriptions, the router automatically determines which protocol to use based on the `Accept` header in the request. When subscribing to subgraphs, the router prefers multipart first, then falls back to SSE.
