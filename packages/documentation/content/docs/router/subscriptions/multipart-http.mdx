...
Apollo's Multipart HTTP protocol is a multipart HTTP-based subscriptions protocol. The router supports it for both client-to-router and router-to-subgraph communication.
...
HTTP subscriptions are enabled automatically when [subscriptions are enabled](/docs/router/subscriptions#configuration). No additional configuration is needed to use multipart HTTP on the client side.
