...
<a name="select-a-supergraph-for-the-request"></a>
...
its schema first. Both constructors accept query-planner options only. Router configuration,
telemetry, subgraph executors, and schema-aware caches remain router-owned.
...
- [`OnGraphqlAnalysisHookPayload`](https://github.com/graphql-hive/router/blob/main/lib/executor/src/plugins/hooks/on_graphql_analysis.rs#:~:text=struct%20OnGraphqlAnalysisHookPayload)
- [`OperationFilter`](https://github.com/graphql-hive/router/blob/main/lib/executor/src/operation_filter.rs#:~:text=struct%20OperationFilter)
