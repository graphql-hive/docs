...
| Field           | Type       | Default  | Required | Notes                                                     |
| --------------- | ---------- | -------- | -------- | --------------------------------------------------------- |
| `storage_id`    | `string`   | -        | yes      | ID of a storage configured in top-level `storages`.       |
| `location`      | `string`   | -        | yes      | Object key/path of the persisted documents manifest.      |
| `poll_interval` | `duration` | disabled | no       | Poll storage for updated manifest content.                |

Storage-based manifests use the same parsing behavior and manifest formats as file-based manifests.

If `storage_id` does not reference an existing entry in `storages`, router startup fails.

When `poll_interval` is set, reloads use storage change detection (for example ETag/not-modified
behavior where available) to avoid unnecessary manifest reloads.
