...
- `.request.path_params` - path parameters captured from `http.graphql_endpoint` patterns (for
  example, with `/{tenant}/graphql`, `.request.path_params.tenant` contains the matched value).
