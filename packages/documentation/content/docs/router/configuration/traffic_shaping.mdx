...
Configures router-level in-flight request deduplication. When enabled, identical concurrent
incoming GraphQL `query` requests are coalesced into a single execution, with the result shared
among all waiting clients. This is distinct from the subgraph-level `dedupe_enabled`, which
...
Enables or disables router-level in-flight request deduplication.
