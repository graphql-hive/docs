...
<Callout type="warning">
  **Use `info` in production.** Raise the level to `debug` only temporarily and locally when you are
  actively investigating an issue. Running `debug` under production traffic can measurably reduce
  throughput and produce very large log volumes.
</Callout>
