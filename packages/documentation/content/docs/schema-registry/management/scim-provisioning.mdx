...
description: Provision Hive Console users and groups from an identity provider using SCIM 2.0.
---
...
SCIM provisioning complements [Single Sign-On (SSO)](./sso-oidc-provider): OIDC authenticates
users, while SCIM controls which users exist, whether they are active, their group memberships, and
...
- A SCIM 2.0 integration in your identity provider that supports user and group provisioning
...
If **User ID Claim** is empty, Hive Console uses the standard OIDC `sub` claim. This works when your
identity provider sends the same value as the SCIM `externalId`.
...
Select the supported provisioning actions shown in the screenshot: **Import New Users and Profile
Updates**, **Push New Users**, **Push Profile Updates**, **Push Groups**, and **Import Groups**.
...
| `customExternalId` | `user.getInternalProperty("id")`                           |
