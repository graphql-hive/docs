...
<Callout type="info">
  ...
  Complexity](/docs/router/security/operation-complexity) limits. Complexity limits stop
  structurally complex queries (deep nesting, many fields); Demand Control stops computationally
  expensive ones — such as a query that requests a massive list — regardless of their shape.
</Callout>
...
demand_control:
  ...
  strategy:
    static_estimated:
      ...
      list_size: 10 # default list size for fields without @listSize
```
...
Beyond the supergraph-wide limit, you can give individual subgraphs their own, tighter budgets. When
a subgraph's portion of the plan is too expensive, only that fetch is skipped — the rest of the plan
still runs and the response comes back partial.
