...
<Lede>
  ...
  requests flow through a single front door: Hive Gateway.
</Lede>
...
ActiveRecord data loading. Then they put Hive Gateway in front of both stacks and migrated query by
...
_Phase 2 — the migration: Hive Gateway in front of both stacks, moving traffic query by query with
...
entirely out of The Guild's open source tooling.
...
What Console adds on top of a pipeline like Reverb's is everything that's hard to hand-roll: usage
insights down to the individual field and coordinate; conditional breaking-change detection that
judges every schema change not just against static client code, but against real production
traffic, stored trusted documents (persisted queries), and exposed MCP tools; schema-change
notifications to Slack or anywhere a webhook reaches; the new Metric Alerts — threshold and
relative-change alerting on error rates, latency percentiles, and traffic drops, delivered to
Slack, MS Teams, or webhooks, so the platform team hears about an incident before users do; a
schema explorer that gives every engineer — not just the platform team — visibility into the
supergraph: who owns what, and how and by whom it's being used; schema contracts for serving
tailored views of the graph to different consumers; and with Gateway v2's OpenTelemetry support,
distributed tracing of every operation across the gateway and subgraphs.
...
The team's creativity didn't stop there. Reverb wrote a custom Hive Gateway plugin — a "node
...
- When a stubborn request-waterfall was adding over 100ms to the p50 of a high-traffic query, The
  Guild's engineers dug in — and found the bottleneck not in Hive Gateway, but in Reverb's own
...
In April 2026, Reverb's platform team posted this, unprompted, in the shared Slack channel:
...
<cite>
  <span>Reverb platform team</span>
</cite>
...
In May 2026, Reverb completed the upgrade to Hive Gateway v2 in production. Thirty days of Datadog
data later:
...
<LargeCallout
  ...
  variant="primary"
>
  Hive Gateway and the Hive schema registry are fully open source (MIT) — use
  every piece standalone, self-host the whole platform, or start with the hosted
  registry for free.
</LargeCallout>
