...
## `AbortController` and `fetch` on the Client { }
...
<Callout>
  **Note:** When a pending fetch request is canceled, the Promise returned from
  the fetch call will reject with a
  [`DOMException`](https://developer.mozilla.org/en-US/docs/Web/API/DOMException)`.
  It is recommended to always add a catch handler for your fetch calls to avoid
