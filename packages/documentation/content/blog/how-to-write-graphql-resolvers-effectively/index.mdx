...
<Callout emoji="💡" type="info">
  You can learn about GraphQL Scalar, including native Scalar and coercion
...
const resolvers = {
  ...
  Actor: {
    ...
    stageName: (parent: ActorMapper, _, { database }) =>
      database.actors[parent].stageName, // 8.
  },
};
```
...
<Callout emoji="💡" type="info">
  By using **mappers** and **defer resolve** techniques, we avoid unnecessary
  database calls and reduce code duplication. The next time we need to write a
  resolver that returns `Movie`, `Actor` objects, we can simply return the
  corresponding mapper objects.
</Callout>
...
<Callout emoji="💡" type="info">
  Learn how to set up GraphQL Code Generator and Server Preset for GraphQL Yoga
  and Apollo Server in this guide
