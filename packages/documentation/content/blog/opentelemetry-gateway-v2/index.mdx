...
{/* eslint-disable-next-line jsx-a11y/media-has-caption -- demo video, no dialogue */}
<video>
  <source
    src="/blog-assets/opentelemetry-gateway-v2/demo-span-parenting.webm"
    type="video/webm"
  />
  <source
    src="/blog-assets/opentelemetry-gateway-v2/demo-span-parenting.mp4"
    type="video/mp4"
  />
  <a
  ...
  >
    <img alt="Span parenting demo" src="/blog-assets/opentelemetry-gateway-v2/demo-span-parenting.png" />
  </a>
  ...
  genericAuth: {
    ...
    resolveUserFn: (ctx) =>
      hive.tracer!.startActiveSpan("auth.validate_user", async (span) => {
        ...
        const user = await fetch(`https://auth/validate?token=${token}`).then(
          (r) => r.json(),
        );
        span.end();
        return user;
  ...
  plugins: () => [
    {
      onExecute({ context }) {
        trace
          ...
          ?.setAttribute(
            "auth.user.id",
            context.user.id ?? "<unauthenticated>",
          );
      },
    },
  ],
...
visit our [documentation](/docs/gateway).
