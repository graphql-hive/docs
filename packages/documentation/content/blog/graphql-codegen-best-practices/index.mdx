...
<Callout>
  You can use it with any other wrapper of Apollo-Client, such as
  `apollo-angular` or `react-apollo`.
</Callout>
...
<Callout>
  Please have some judgment before creating fragments, it should represent data
  structure that is specific per component. Don't abuse this mechanism by
  creating fragments with a single field. Try to group it in a way that matches
